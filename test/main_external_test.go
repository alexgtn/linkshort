...
import (
	...
	link2 "github.com/alexgtn/go-linkshort/tools/proto"
)
...
func Test_Main(t *testing.T) {
	...
	t.Run("create and redirect", func(t *testing.T) {
		c := link2.NewLinkshortServiceClient(conn)
		...
		for i := 0; i < clientReqCount; i++ {
			go func(long string, i int) {
				...
				l, err := c.CreateLink(ctx, &link2.CreateLinkRequest{LongUri: long})
				if err != nil {
					log.Fatalf("%d could not create link: %v", i, err)
				}
				shortPath := strings.TrimPrefix(l.GetShortUri(), "http://localhost:8080/")

				r, err := c.Redirect(ctx, &link2.RedirectRequest{ShortPath: shortPath})
				if err != nil {
					log.Fatalf("%d could not create link: %v", i, err)
				}
